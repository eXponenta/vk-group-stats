...
const RESERVED = [
    'im', 'groups', 'feed', 'friends', 'video', 'docs', 'apps'
];
...
(function() {
	...
	function updateStatView(stat) {
        ...
        text += crtItm("Постов/сутки", (stat.count / stat.period).toFixed(2))
        text += crtItm("ER", calcER(stat).toFixed(2) + "%", "font-weight:bolder; border-top:1px #939393 solid;");
        ...
        injectedParentElement.querySelector("#data_block > #force_update").addEventListener("click", () =>{
    ...
    function updateStatViewError(res) {
        ...
        data.querySelector("#force_update").addEventListener("click", () =>{
            updateGroupStats(latestGroupID);
        }, false);
    }
		...
		VKREST.groups.getById(name).then(r => {
			if (r.response && r.response.length > 0) {
				...
				if (!injectedParentElement) {
					...
					injectedParentElement.innerHTML = TEMPLATE;
